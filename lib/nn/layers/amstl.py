...
from .litegcgru import LiteGCGRUCell
...
class graph_constructor(nn.Module):
    ...
    def forward(self, predefined_A):
        adjacentA = predefined_A
        adj = torch.sigmoid(self.A)
        InnerAdjacentA = adjacentA.to(self.device)

        adjlist = [adj * InnerAdjacentA]
        # adjlist = [adj]
        return adjlist
...
class Multi_ScaleTCN_MLPAgg(nn.Module):
    def __init__(self, cin, cout, dropout):
        super(Multi_ScaleTCN_MLPAgg, self).__init__()
        self.tconv = nn.ModuleList()
    ...
    def forward(self,input):
        ...
        # i_input = nn.functional.pad(input, (int(zero_length[-1] / 2), zero_length[-1] - int(zero_length[-1] / 2)))
        # x1 = self.tconv[-1](i_input)
        return x1

class AMSTL(nn.Module):
    def __init__(self,
                 input_size,
                 hidden_size,
                 u_size=None,
        ...
        for i in range(self.n_layers):
            self.cells.append(LiteGCGRUCell(d_in=rnn_input_size if i == 0 else self.hidden_size,
        ...
        self.temporal_aggre = Multi_ScaleTCN_MLPAgg(cin=self.hidden_size,cout=self.hidden_size, dropout=dropout)
        ...
        self.graph = graph_constructor(n_nodes, 64, 'cuda:0')
        self.graph2 = graph_constructor(n_nodes, 64, 'cuda:0')
    ...
    def forward(self, x, adj, mask=None, u=None, h=None, cached_support=False):
        ...
        *_, steps = x.size()
        ...
        imputed_h.append(h[-1])

        imputed_x = torch.stack(imputed_x, dim=-1)
        imputed_h = torch.stack(imputed_h, dim=-1)
        predictions = torch.stack(predictions, dim=-1)
        # # feature fusion on multi-scale resolution.
        imputed_h = self.temporal_aggre(imputed_h)

        # # task-specific interaction extration
        imputations, representations,_ = self.spatial_decoder(x=imputed_x, m=mask, h=imputed_h[...,:-1], u=u, spa_adj=adj2,
                                                            cached_support=cached_support)
        return imputations, predictions, representations, states

class BiAMSTL(nn.Module):
    def __init__(self,
                 input_size,
                 hidden_size,
                 ff_size,
                 ...
                 layer_norm=False,
                 merge='mlp'):
        super(BiAMSTL, self).__init__()
        self.fwd_amstn = AMSTL(input_size=input_size,
        ...
        self.bwd_amstn = AMSTL(input_size=input_size,
                            hidden_size=hidden_size,
                            n_layers=n_layers,
                            dropout=dropout,
                            n_nodes=n_nodes,
    ...
    def forward(self, x, adj, mask=None, u=None, cached_support=False):
        ...
        # Forward
        fwd_out, fwd_pred, fwd_repr, _ = self.fwd_amstn(x, supp, mask=mask, u=u, cached_support=cached_support)
        # # Backward
        rev_x, rev_mask, rev_u = [reverse_tensor(tens) for tens in (x, mask, u)]
        *bwd_res, _ = self.bwd_amstn(rev_x, supp, mask=rev_mask, u=rev_u, cached_support=cached_support)
