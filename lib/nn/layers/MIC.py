...
class FeedForwardNetwork(nn.Module):
    def __init__(self, hidden_size, filter_size, dropout_rate=0.1):
        ...
        self.layer1 = nn.Conv2d(hidden_size, filter_size, kernel_size=(1,1))
        self.relu = nn.ReLU()

        self.dropout = nn.Dropout(dropout_rate)
        self.layer2 = nn.Conv2d(filter_size, hidden_size, kernel_size=(1,1))
...
class MIC(nn.Module):
    ...
    def conv_trans_conv(self, input, conv1d, conv1d_trans, isometric):
        batch, channel, nodes, length = input.shape
        x = input
        ...
        # # isometric convolution
        # zeros = torch.zeros((x.shape[0], x.shape[1], x.shape[2], x.shape[3] - 1), device=self.device)
        # x = torch.cat((zeros, x), dim=-1)
        # x = self.act(isometric(x))
        # # x = self.drop(x)
        # # x = self.norm((x + x1).permute(0, 3, 2, 1)).permute(0, 3, 2, 1)

        # upsampling convolution
        x = self.act(conv1d_trans(x))
        # x = self.drop(x)
        x = x[:, :, :, :length]  # truncate
        # x = self.norm((x + input).permute(0, 3, 2, 1)).permute(0, 3, 2, 1)
        # x = self.norm(x + input)
        return x
    ...
    def forward(self, src):
        ...
        b, f, o, n, l = mg.shape
        mg2 = rearrange(mg, 'b f o n l -> b f o (n l)')
        mg2 = self.merge(mg2).squeeze(-2)
        mg = rearrange(mg2, 'b f (n l) -> b f n l', n=n, l=l)
        return mg
        # return self.fnn_norm((mg + self.fnn(mg)).permute(0, 3, 2, 1)).permute(0, 3, 2, 1)
