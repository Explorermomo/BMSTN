...
class LiteGCGRUCell(nn.Module):
    """
    Lite Graph Convolution Gated Recurrent Unit Cell.
    """
    ...
    def __init__(self, d_in, num_units, support_len, order, activation='tanh'):
        ...
        """
        super(LiteGCGRUCell, self).__init__()
    ...
    def forward(self, x, h, adj):
        ...
        # # Lite Gated Recurrent Unit
        r = torch.tanh(self.forget_gate(x_h, adj))
        x_c = torch.cat([x, r * h], dim=1)
        f = self.activation_fn(self.c_gate(x_c, adj))
        abs_r = torch.abs(r)
        out = abs_r * h + (1. - abs_r) * f

        return out


class LiteGCRNN(nn.Module):
    def __init__(self,
                 d_in,
                 d_model,
                 d_out,
        ...
        for i in range(self.n_layers):
            self.rnn_cells.append(LiteGCGRUCell(d_in=self.d_in if i == 0 else self.d_model,
