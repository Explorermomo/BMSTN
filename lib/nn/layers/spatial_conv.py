...
class SpatialConvOrderK(nn.Module):
    ...
    def __init__(self, c_in, c_out, support_len=3, order=1, include_self=True, alpha=0.00):
    ...
    def forward(self, x, support, alpha=0.05):
        ...
        for a in support:
            x1 = self.alpha * x + (1-self.alpha) * torch.einsum('ncvl,wv->ncwl', (x, a)).contiguous()
            out.append(x1)
            for k in range(2, self.order + 1):
                x2 = self.alpha * x + (1-self.alpha) * torch.einsum('ncvl,wv->ncwl', (x1, a)).contiguous()
                out.append(x2)
                x1 = x2
...
class TemporalConvOrderK(nn.Module):
    """
    Spatial convolution of order K with possibly different diffusion matrices (useful for directed graphs)
    ...
    def __init__(self, c_in, c_out, support_len=3, order=1, include_self=True):
        super(TemporalConvOrderK, self).__init__()
        self.include_self = include_self
        c_in = (order * support_len + (1 if include_self else 0)) * c_in
        self.mlp = nn.Conv2d(c_in, c_out, kernel_size=1)
        self.order = order
    ...
    def forward(self, x, support):
        ...
        if (type(support) is not list):
            support = [support]
        for a in support:
            x1 = torch.einsum('ncvl,wl->ncvw', (x, a)).contiguous()

            out.append(x1)
            for k in range(2, self.order + 1):
                x2 = torch.einsum('ncvl,wl->ncvw', (x1, a)).contiguous()
                out.append(x2)
                x1 = x2
