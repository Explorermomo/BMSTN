...
class AMSTNet(nn.Module):
    def __init__(self,
        ...
        self.biamstl = BiAMSTL(input_size=self.d_in,
                              ff_size=d_ff,
    ...
    def forward(self, x, mask=None, u=None, **kwargs):
        ...
        imputation, prediction = self.biamstl(x, self.adj, mask=mask, u=u, cached_support=self.training)
        # imputation, prediction = self.bigcin(x, self.adj, mask=mask, u=u, cached_support=self.training)
