...
class Weather(PandasDataset):
    def __init__(self, impute_zeros=False, freq='10T'):

        df, dist, mask = self.load(impute_zeros=impute_zeros)
        self.dist = dist
        super().__init__(dataframe=df, u=None, mask=mask, name='weather', freq=freq, aggr='nearest')

    def load(self, impute_zeros=True):
        path = os.path.join(datasets_path['weather'], 'weather.csv')
        df = pd.read_csv(path)

        # datetime_idx = sorted(df.iloc[:,0])
        # date_range = pd.date_range(datetime_idx[0], datetime_idx[-1], freq='10T')
        # df = df.iloc[:,1:]
        # df.index = pd.to_datetime(date_range)

        datetime_idx = sorted(df.iloc[:,0])
        date_range = pd.date_range(datetime_idx[0], datetime_idx[-1], freq='10T')
        df = df.iloc[:, 1:]
        df.index = date_range[:df.shape[0]]
    ...
    def get_similarity(self, thr=0.1, force_symmetric=False, sparse=False):
        if self.dist is None:
            Total_Nodes = self.df.shape[1]
            adj = np.ones([Total_Nodes,Total_Nodes])
            if sparse:
...
class MissingValuesWeather(Weather):
    SEED = 9101112

    def __init__(self, p_fault=0.0015, p_noise=0.05): # p_fault=0.0015 , p_noise=0.05
        super(MissingValuesWeather, self).__init__(impute_zeros=True)
