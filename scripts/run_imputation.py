...
def has_graph_support(model_cls):
    return model_cls in [models.GRINet, models.AMSTNet, models.MPGRUNet, models.BiMPGRUNet]
...
def get_model_classes(model_str):
    ...
    elif model_str == 'amstn':
        model, filler = models.AMSTNet, fillers.GraphFiller
    elif model_str == 'graphwavenet':
        model, filler = models.gwnet, fillers.GraphFiller
    else:
...
def get_dataset(dataset_name):
    ...
    elif dataset_name == 'air437_block':
        dataset = datasets.MissingValuesAirQuality(impute_nans=True, small=False)
    elif dataset_name == 'weather_block':
        dataset = datasets.MissingValuesWeather()
    elif dataset_name == 'la_point':
        dataset = datasets.MissingValuesMetrLA(p_fault=0., p_noise=0.25)
    elif dataset_name == 'weather_point':
        dataset = datasets.MissingValuesWeather(p_fault=0., p_noise=0.25)
    elif dataset_name == 'bay_point':
        dataset = datasets.MissingValuesPemsBay(p_fault=0., p_noise=0.25)
    elif dataset_name == 'air437_point':
        dataset = datasets.MissingValuesAirQuality(impute_nans=True, small=False, p_fault=0., p_noise=0.25)
    else:
...
def parse_args():
    ...
    parser.add_argument('--whiten-prob', type=float, default=0.05) # 0.05
...
def run_experiment(args):
    ...
    # model's inputs
    # additional_model_hparams = dict(adj=adj, d_in=dm.d_windowin, n_nodes=dm.n_nodes)
